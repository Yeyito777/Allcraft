...
boot_jdk="$repo_root/jvm/linux-x64"
output="$repo_root/build/jvm/linux-x64"
install=false
...
image="$source_root/build/linux-x86_64-server-release/images/jdk"
modules="$("$image/bin/java" --list-modules | sed 's/@.*//' | paste -sd, -)"
[[ -n "$modules" ]] || { echo "Built JDK reported no modules" >&2; exit 1; }
rm -rf "$output"
mkdir -p "$(dirname "$output")"
"$image/bin/jlink" \
    --module-path "$image/jmods" \
    --no-man-pages \
    --compress=zip-6 \
...
"$output/bin/java" -version
"$output/bin/javac" -version
